...
def url_parser(url: str):
    url = url.strip()
    for shop in SHOPS:
        url_re = r"(^https://www\.|^http://www\.|^www\.)?" + SHOPS[shop][0]
...
def _simple_tags(soup: BeautifulSoup, shop: str):
    ...
    item = soup.find(SOUP_TAGS[shop]["item"][0], attrs=SOUP_TAGS[shop]["price"][1]).text.strip()
    price_raw = soup.find(SOUP_TAGS[shop]["price"][0], attrs=SOUP_TAGS[shop]["price"][1]).text
